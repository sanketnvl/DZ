...
public class JerseyFileUpload extends SpringBeanAutowiringSupport{
									...
									try {
										...
										if(doc.getOriginal() !=null){
											...
											if(updateStatus != 1){
												...
												String url = SERVER_URL+"app/applicationsigndocument?token="+token+"&envelopeid="+envelopeId+"&email="+document.getSignerEmail()+"&signername="+document.getSignerName()+userUrl+"&redirecturl="+returnUrl;
												document.setUrl(url);
											}
	...
	public Response downloadDocument(@Context HttpServletRequest request,String documentId)
	{
		String ipAddress = request.getHeader("X-FORWARDED-FOR");
		int userId = 0;
		if (ipAddress == null) {
		...
		try {
			...
			audit.setUserId(userId);
			audit.setIpAddress(ipAddress);
			audit.setWebRequest(documentId);
					...
					if(fileUtils.checkValidPdf(fileName))
					{
						...
						if(inputStream !=null)
						{
							...
							if(user != null)
							{
								int uploadFlag;
								try {
									uploadFlag = jerseyService.createDocument(user.getId(),envelopeId,fileName,inputStream,status,signType,document.getSubject(),document.getMessage(),clientIPAddress);
									if(uploadFlag > 0)
									{
										msg = "success";
									}else{
